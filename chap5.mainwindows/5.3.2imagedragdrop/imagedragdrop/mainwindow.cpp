...
{
    ui->setupUi(this);
    //add by sunzhong 20180708 for 自定义拖放
    ...
    label->move(100, 100);
    label->setAttribute(Qt::WA_DeleteOnClose);  //当窗口关闭时，销毁图片
    //end of add
}
...
void MainWindow::mousePressEvent(QMouseEvent * event)
{
    ...
    //第三步：将数据放入QMineData中
    QMimeData * mimeData = new QMimeData;   //创建QMimeData用于存放要移动的数据
    ...
    //第五步：给原图片添加阴影
    QPixmap tempPixmap = pixmap;        //是原图片添加阴影
    ...
    if(drag->exec(Qt::CopyAction | Qt::MoveAction, Qt::CopyAction)
        ...
        child->close();             //如果是移动操作，则拖放完成后关闭原标签
    else
    {
        child->show();              //如果是复制操作，则拖放完成后显示标签
...
void MainWindow::dragEnterEvent(QDragEnterEvent * event)
{
    ...
    if(event->mimeData()->hasFormat("myimage/png")) //自定义的MINE类型"myimage/png"
    {
