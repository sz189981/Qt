...
void MainWindow::findNext() //查找下一个
{
    ...
    if(isfind)  //如果查找成功，则输出字符串所在行和列的编号
    {
        qDebug()<<tr("行号：%1 列好：%2")
    ...
    ui(new Ui::MainWindow)
{
    ...
    frameFormat.setBorderStyle(
                QTextFrameFormat::BorderStyle_Dotted);  //设置边框样式
    ...
    connect(action_font, &QAction::toggled,
    ...
    QVBoxLayout * layout = new QVBoxLayout; //创建垂直布局管理
