    ...
    ui(new Ui::MainWindow)
{
    ...
    //13.2(3)①添加菜单栏断开槽函数
    connect(ui->DisCommunated, SIGNAL(triggered()),
            this, SLOT(MainWindow_DisCommunated()));
    ...
    ui->SendDataPush->setEnabled(false);

    //12.2(6)初始化时自动将发送按钮屏蔽，等串口连接上再开启
...
//12.1(2)添加手动发送槽函数(3)添加手动发送处理程序
void MainWindow::on_SendDataPush_clicked()
{
    P_Com->WorkSendMessage();
}

//12.2(3)声明、实现定时发送槽函数②
void MainWindow::SlectAutoSend()   //定时发送
{
    P_Com->WorkSendMessage();
}

//12.2(4)添加自动发送槽函数，选择toggled(bool)
//(5)添加自动发送处理程序
void MainWindow::on_AutoSend_toggled(bool checked)
{
    ...
    {
        myTime->stop();
        ui->SendDataPush->setEnabled(true);
    }
}

//13.1(1)(2)添加清除发送槽函数处理程序
void MainWindow::on_SendDataClear_clicked()
{
    ui->SendHeadLineEdit->setText("");
    ui->SendDatatextEdit->setText("");
    ui->SendCRCLineEdit->setText("");
    ui->SendLastLineEdit->setText("");
}

//13.2(4)实现断开槽函数
#if 1
void MainWindow::MainWindow_DisCommunated()
{
#if 1
    if(P_Com != NULL)
    {
...
//13.3(2)关闭窗口函数实现
#if 1
void MainWindow::closeEvent(QCloseEvent *event)        //重写虚拟关闭函数
{
    int ret = QMessageBox::warning(this, tr("Warning"), tr("确认退出系统？"),
