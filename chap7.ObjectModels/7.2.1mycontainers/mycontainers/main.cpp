...
int main(int argc, char *argv[])
{
    ...
    //add by sunzhong 201807010 for QList
    QList<QString> list;
    list<<"aa"<<"bb"<<"cc"; //插入项目
    if(list[1] == "bb")
        list[1] = "ab";
    list.replace(2, "bc");  //将"cc"替换为"bc"
    qDebug()<<"the list is: ";
    for(int i = 0; i < list.size(); ++i)
    {
        qDebug()<<list.at(i);   //输出列表内容,现在列表为aa ab bc
    }
    ...
    //第一个"mm"的位置，默认从位置0开始向前查找，返回第一个匹配的项目的位置
    qDebug()<<"the first 'mm' index: "<<list.indexOf("mm");
    //第二个"mm"的位置，我们指定从位置1开始向前查找
